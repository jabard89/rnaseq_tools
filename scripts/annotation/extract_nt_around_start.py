...
@author: Jared Bard
Based on extract_first_nts_from_UTR.py, extracts nts x before and y after the start codon
...
if __name__=='__main__':
    ...
    parser.add_argument(dest="u",default=25,type=int,help="how many nts to extract upstream of the starting A")
    parser.add_argument(dest="d",default=24,type=int,help="how many nts to extract downstream of the starting A")
    ...
    #                 '/home/jabard89/Dropbox/code_JB/repos/rnaseq_tools/test.txt','25','24'])
    ...
    for gene in gff_dict:
        if gff_dict[gene]['CDSs']:
            strand=gff_dict[gene]['CDSs'][0].iv.strand
            chrom=gff_dict[gene]['CDSs'][0].iv.chrom
            CDS_starts = [x.iv.start_d for x in gff_dict[gene]['CDSs']]
            if strand == "+":
                start = min(CDS_starts)
                start_iv = HTSeq.GenomicInterval(chrom,start-args.u,start+args.d+1,strand)
            elif strand == "-":
                start = max(CDS_starts)
                start_iv = HTSeq.GenomicInterval(chrom,start-args.d,start+args.u+1,strand)
            out_dict[gene] = {"ORF":gene,
                              "-"+str(args.u)+":+"+str(args.d):extract_seq_fromiv(start_iv,chrom_dict)}
