...
if __name__=='__main__':
    ...
    for gene in gff_dict:
        ...
        if gff_dict[gene]['5UTR']:
            out_dict[gene]['UTR5'] = extract_seq_fromiv(gff_dict[gene]['5UTR'][0].iv,chrom_dict)
            ivplus = gff_dict[gene]['5UTR'][0].iv.copy()
            ivplus.length = ivplus.length + args.add_nts
            out_dict[gene]['UTR5_plus' + str(args.add_nts)] = extract_seq_fromiv(ivplus,chrom_dict)
        if gff_dict[gene]['3UTR']:
            out_dict[gene]['UTR3'] = extract_seq_fromiv(gff_dict[gene]['3UTR'][0].iv,chrom_dict)
