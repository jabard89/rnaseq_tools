library(tidyverse)
working.dir <- "/home/jbard/beagle3-dadrummond/jbard/211216/snake"
gene_labels <- read_tsv(paste0(working.dir,"/src/201014_labeled_genes_scer.txt")) %>%
  mutate(length.CDS=length.prot*3)

files <- list.files(paste0(working.dir,"/counts"),
                          pattern="dist.tsv",full.names = T,recursive=TRUE)

names <- str_extract(files,pattern="(?<=211216_STAR_).*(?=_dist\\.tsv)")
...
t$files %>% map2(t$names,function(file,name) {
  ...
  d_binned_byORF <- d_counts_binned %>%
    ...
    write_tsv(paste0("counts/",name,"/211216_counts_bin100_",name,".tsv.gz"))
})
